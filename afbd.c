...
static int __init afd_init(void) {
    ...
    nsectors = (sa_cyl - 1) * sa_spt * (aheads);      // Device reserved area usable space in sectors. We'll skip one cylinder here.
    ...
    if (Queue == NULL)
        goto out;
    blk_queue_phy_block_size(Queue, phy_block_size);
    /*
     * Get registered.
     */
...
static struct pchs xlate_pba(sector_t _pba) {
    ...
    int* _lhead = &(lheads) + phy_h;
    chs.h = *_lhead;
    chs.s = (short)((pba % sa_spt) + 1);
    //printk(KERN_NOTICE "afd: PBA=%d, CHS=%d,%hd,%hd.\n", pba, chs.c, chs.h, chs.s);
    return chs;
}
...
static int set_logical_heads(int headmask) {
    ...
    for (i=0; i<8; i++) {
         _lheads_x = &(lheads) + i;
        (*_lheads_x) = -1; //Initialize all heads to -1.
        int h = 1 << i;
        if (headmask & h) {
            if (skipped++ >= 2) {
                _lheads_x = &(lheads) + curr;
                (*_lheads_x) = i;
                ++curr;
            }
        }
...
static int get_dev_family_code(char* model, char* family_code) {
    ...
    hyphen = strchr(model, '-');
    if (hyphen != NULL) {
        size_t fullfam_len = strlen(hyphen);
        if (fullfam_len == 6) {
           memcpy(family_code, hyphen, 2);
           return 2;
        } else if (fullfam_len == 7) {
           memcpy(family_code, hyphen, 3);
           return 3;
        }
    }
    return 0;
